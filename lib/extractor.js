...
function processFile(filePath, fileSize, outerCategory) {
    logLine('Saving file: ' + filePath);
    var newFile = {
        path:             filePath,
        size:             fileSize,
        archive:          _archive,
        subject_category: determineCategory(filePath, SUBJECTS),
        grade_category:   outerCategory,
        created_at:       new Date()
    };
    return _models.File.findOneAndUpdate(
        { path: filePath },
        newFile,
...
function decodeDir(encodeFrom, encodeTo, dir) {
    return new Promise(function(resolve, reject) {
        var reencodeProcess = childProcess.spawn('convmv', ['-f', encodeFrom, '-t', encodeTo, '-r', '--notest', dir]);
        reencodeProcess.stdout.on('data', function(data) {
            var str = data.toString();
            logLine('convmv says:');
            logLine(str);
        });
        reencodeProcess.stderr.on('data', function(data) {
            var str = data.toString();
            logLine('convmv complains:');
            logLine(str);
        });
        reencodeProcess.on('close', function(code) {
            if (code === 0 || code === 255) {
                resolve(code);
            } else {
                reject(code);
...
function reencode(dir) {
    return decodeDir('utf8', 'ISO-8859-1', dir).then(function(err) {
        if (err === 0) {
            logLine('convmv first done.');
            return decodeDir('gbk', 'utf8', dir);
        } else if (err === 255) {
...
module.exports = function(models, archive, password, logLineCallback, logErrorCallback) {
    ...
    return extract(
        ...
        logLine('Trying to reencode...');
        return reencode(tmpDest);
    }).then(function() {

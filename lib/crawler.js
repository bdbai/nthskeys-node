...
function getTextRecursively(el, maxDepth) {
...
function getMainPage(page) {
    return new Promise(function(resolve, reject) {
        ...
        try {
            var req = http.request(reqParams, responseCallback);
...
function getEntries(listBody) {
    ...
    for (var i = 0; i < $elems.length; i++) {
        var $elem = $elems[i];
        var $title = getTextRecursively($elem);
...
function fetchAttachments(entry) {
    return getFullLink(entry.url).bind({}).then(function(body) {
        ...
        var $textElem = getTextRecursively($hrefElem);
        this.attachment =  {
            title:           $textElem.data,
        ...
        return new Promise(function(resolve, reject) {
            ...
            var req = http.request({
                ...
                var interval = setInterval(function() {
                    logLine('Still fetching archive: ' + that.attachment.title);
                }, 60000);
                res.pipe(stream);
                res.on('end', function() {
...
module.exports = function(_model) {
    ...
    var promise = Promise.all([
        ...
        getMainPage(process.env.CRAWLER_PAGE || 1),
        ...
        this.latestEntryTime = entries.reduce(function(prev,curr) {
            if (curr.time > lastEntryTime &&
                curr.title.indexOf(process.env.CRAWLER_KEYWORD || '暑假作业') !== -1
            ) {
